...
class WellSample extends Model
{
    ...
    public function scopeCrosstab($query, $wellID)
    {
        return $query->select('sampleDate', \DB::raw("
                    ...
                    max(if(chemID=10, pfcLevel, ' ')) as 'PFDeA', max(if(chemID=10, noteAbr, ' ')) as 'PFDeANote',
                    max(if(chemID=11, pfcLevel, ' ')) as 'PFDoA', max(if(chemID=11, noteAbr, ' ')) as 'PFDoANote',
                    max(if(chemID=12, pfcLevel, ' ')) as 'PFHpS', max(if(chemID=12, noteAbr, ' ')) as 'PFHpSNote',
                    max(if(chemID=13, pfcLevel, ' ')) as 'PFHpA', max(if(chemID=13, noteAbr, ' ')) as 'PFHpANote',
                    max(if(chemID=14, pfcLevel, ' ')) as 'PFNA', max(if(chemID=14, noteAbr, ' ')) as 'PFNANote',
                    max(if(chemID=15, pfcLevel, ' ')) as 'PFPeA', max(if(chemID=15, noteAbr, ' ')) as 'PFPeANote',
                    max(if(chemID=16, pfcLevel, ' ')) as 'PFTeDA', max(if(chemID=16, noteAbr, ' ')) as 'PFTeDANote',
                    max(if(chemID=17, pfcLevel, ' ')) as 'PFTrDA', max(if(chemID=17, noteAbr, ' ')) as 'PFTrDANote',
                    max(if(chemID=18, pfcLevel, ' ')) as 'PFUnA', max(if(chemID=18, noteAbr, ' ')) as 'PFUnANote',
                    max(if(chemID=19, pfcLevel, ' ')) as 'FTS62', max(if(chemID=19, noteAbr, ' ')) as 'FTS62Note',
                    max(if(chemID=20, pfcLevel, ' ')) as 'FTS82', max(if(chemID=20, noteAbr, ' ')) as 'FTS82Note',
                    max(if(chemID=21, pfcLevel, ' ')) as 'EtFOSA', max(if(chemID=21, noteAbr, ' ')) as 'EtFOSANote',
                    max(if(chemID=22, pfcLevel, ' ')) as 'EtFOSE', max(if(chemID=22, noteAbr, ' ')) as 'EtFOSENote',
                    max(if(chemID=23, pfcLevel, ' ')) as 'MeFOSA', max(if(chemID=23, noteAbr, ' ')) as 'MeFOSANote',
                    max(if(chemID=24, pfcLevel, ' ')) as 'MeFOSE', max(if(chemID=24, noteAbr, ' ')) as 'MeFOSENote'
                "))
